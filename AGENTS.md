...
- **Store lokal**: Dexie/IndexedDB `src/lib/offline/db.ts` (`fvms-offline`, skema saat ini v3): tabel `schedules` (join denormalisasi `*_name`), `visitNotes` (pk `schedule_id`), `visitPhotos` (+ blob lokal utk foto belum upload), `regions` (kab/kec/desa, key `entity:id`), `outbox` (antrian mutasi idempotent: `table/action/entity_id/payload`), `meta` (watermark `watermark:<table>:<userId>`, `last_sync_at`).
...
- Build otomatis: `.github/workflows/android.yml` (tag `android-*` atau manual `workflow_dispatch`) → `./gradlew :app:assembleRelease :app:bundleRelease` → **APK + AAB**. `versionCode` = jumlah commit git (`git rev-list --count HEAD`, butuh `fetch-depth: 0`); `versionName` = tag `android-*` terbaru. Artifact diupload dengan nama `fvms-release-apk-v<version>` & `fvms-release-aab-v<version>`, ditambah **GitHub Release `latest`** (ncipollo/release-action) yang mempublikasikan APK + AAB sebagai rilis bergerak (selalu versi terbaru). **Keystore permanen** sudah disimpan di secrets `TWA_KEYSTORE_B64` / `TWA_KEYSTORE_PASSWORD` / `TWA_KEY_ALIAS` (FP signing stabil, jangan regen!).
...
Transisi status **terbatas** (bukan bebas) — lihat `STATUS_TRANSITIONS` di `src/lib/constants/status.ts`:
...
`gagal_total` → terminal (tidak ada transisi keluar).
...
Setiap perintah / bug / task harus memanggil skill `available_skills` yang paling sesuai **sebelum** mengerjakan, agar pendekatan & standar skill digunakan. Pemetaan task → skill:

| Kategori task | Skill |
| --- | --- |
| Presentasi / PPTX (`proposal/*.pptx`) | `pptx-generator` |
| Tulis / perbarui dokumentasi `.md` (docs, AGENTS) | `code-documenter` |
| Pahami / reverse-engineer kode eksisting | `spec-miner` |
| Kode Next.js App Router | `nextjs-developer` |
| Komponen React | `react-expert` |
| Android TWA / build / CI | `android-native-dev`, `devops-engineer` |
| Desain & dokumentasi API | `api-designer` |
| Keamanan & review keamanan | `secure-code-guardian`, `security-reviewer` |
| Review kode & pengujian | `code-reviewer`, `test-master` |
| SQL / optimasi database | `postgres-pro` |
| Arsitektur & requirement | `architecture-designer`, `feature-forge` |

Catatan: bila skill loader gagal (error "ripgrep execution failed"), baca langsung `SKILL.md` di `/public/.config/opencode/skills/<nama>/SKILL.md` dan ikuti panduannya.
