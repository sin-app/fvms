...
interface ProposalGpsProps {
  defaultLatitude?: number | null;
  defaultLongitude?: number | null;
  defaultAccuracy?: number | null;
}

export function ProposalGps({
  defaultLatitude,
  defaultLongitude,
  defaultAccuracy,
}: ProposalGpsProps) {
  ...
  const [location, setLocation] = useState<{
    ...
    defaultLatitude != null && defaultLongitude != null
      ? { latitude: defaultLatitude, longitude: defaultLongitude, accuracy: defaultAccuracy ?? null }
      : null,
  );
    ...
    <div className="space-y-3">
      ...
      {!location && !error && (
        <p className="text-sm text-muted-foreground">
          Klik &ldquo;Ambil Lokasi&rdquo; untuk merekam posisi lahan yang diajukan.
        </p>
      )}
    </div>
  );
