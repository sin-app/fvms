...
export async function GET(request: Request) {
  try {
    ...
    let query = admin
      .from("schedules")
      .select(
        "id, visit_date, status, kabupaten_id, kecamatan_id, desa_id, member_name, block_no, no_plot, nis, created_at, updated_at, user_id",
        { count: "exact" },
      )
      .is("deleted_at", null)
    ...
    if (error) {
      return withCors(
        request,
        NextResponse.json({ error: "Gagal memuat data jadwal" }, { status: 500 }),
      );
    }
