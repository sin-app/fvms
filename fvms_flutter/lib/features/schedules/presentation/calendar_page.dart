              ...
              return Column(children: [
                ...
                Expanded(
                  child: Builder(builder: (_) {
                    if (_selected == null) {
                      if (events.isEmpty) return const EmptyState(message: 'Belum ada jadwal');
                      return const Center(child: Text('Pilih tanggal'));
                    }
                    final key = _selected!.toIso8601String().substring(0,10);
                    final day = events.where((e) => e.visitDate == key).toList();
                    if (day.isEmpty) return const EmptyState(message: 'Tidak ada jadwal pada tanggal ini');
